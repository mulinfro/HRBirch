...
class Entry(object):
	...
	def getProcessedLs(self,ety,flag):
		...
		if len(idx)==0 or (self.N >1 and ety.N >1):
			idx = range(0,gv.Dim)
		fractor = gv.Dim/(len(idx)+0.0)
		...
		if flag=='nead_ss':
			...
			return (fractor,ls1,ls2,ss1,ss2)
	...
	def d2(self,ety):
		...
		return np.sqrt(fractor*np.sum(rs))
	...
	def d3(self,ety):
		...
		return fractor*np.sqrt(rs/((self.N + ety.N)*(self.N + ety.N-1)))
	...
	def d4(self,ety):
		...
		ls = ls1 + ls2
		ss = ss1 + ss2
		return fractor*(f(ss,ls,self.N+ety.N) - f(ss1,ls1,self.N) \
				- f(ss2,ls2,ety.N))

	distance = d2

	def getRadius(self):
		return np.sqrt((self.lss/self.N - (self.lls/self.N)**2)) /(gv.cftree.Normalizor)
...
class CFTree():
	...
	def updateNormalizer(self):
		...
		for e in self.root.entries:
			...
			tss += e.lss
			tn += e.N
		gv.cftree.Normalizor = self.standard_deviation()(tls,tss,tn)
		gv.cftree.Normalizor[ gv.cftree.Normalizor < 0.01] = 1
	...
	def initClusting(self,iniData = None):
		...
		if gv.offNormal:
			...
			gv.cftree.Normalizor = self.standard_deviation()(ls,ss,len(iniData))
