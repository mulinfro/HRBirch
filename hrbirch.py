...
# may be something wrong, every dimension have normalized, how to selction;
	...
	def incr_radiu(self,ety):
		tmp = Entry(n=0,idx = [])
		tmp.update(self)
		tmp.update(ety)
		res = tmp.lss/tmp.N - (tmp.lls/tmp.N)**2
		return np.sqrt(np.sum(res)/gv.Dim) 
...
class Node(Entry):
	...
	def findClosestEntryPair(self):
		if len(self.entries) < 2: return (-1,None,None)
		lr = xrange(len(self.entries))
		lr = [(i,j) for i in lr for j in lr if i<j]
	...
	def insertEntry(self,ety):
		...
		if closet.child != None:
			...
			if dontSplit:
				...
				return (len(self.entries) <= gv.MAX_ENTRIES_NUM,False)
		elif closet.N +ety.N <= gv.ENTRY_LIMIT and closet.incr_radiu(ety) <= gv.cftree.threshold:
			closet.update(ety)
			closet.selectDimension()
			return (True,False)
		else:
...
class CFTree():
	...
	'''
	def ifReachEntryLimit(self,ety):
		self.fruit.append(ety)
		self.leaves.remove(ety)
		self.leafNum -= 1
		def dfs(node):
			for e in node.entries:
				if id(e) == id(ety):
					...
					if dfs(e.child):
						e.updateMinus(ety)
						if e.N == 0: node.entries.remove(e)
						return True
			return False
		dfs(self.root)
	'''
